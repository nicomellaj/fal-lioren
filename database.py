...
def order_exists(fal_order_id: str) -> bool:
    conn = get_conn()
    row = conn.execute(
        "SELECT id FROM orders WHERE fal_order_id=?", (fal_order_id,)
    ).fetchone()
    conn.close()
...
def save_order(order: dict):
    ...
    conn.execute("""
        ...
        (fal_order_id, fal_order_number, buyer_name, total_amount,
         status, items_json, created_at, delivered_at)
        VALUES (?,?,?,?,?,?,?,?)
    """, (
        order["order_id"],
...
def update_boleta(fal_order_id: str, folio: str, pdf_url: str, status: str = "emitida"):
    conn = get_conn()
    conn.execute("""
        UPDATE orders SET
            boleta_status=?, boleta_folio=?, boleta_pdf_url=?, processed_at=?
        WHERE fal_order_id=?
...
def mark_error(fal_order_id: str, msg: str):
    conn = get_conn()
    conn.execute("""
        UPDATE orders SET boleta_status='error', processed_at=?
        WHERE fal_order_id=?
...
def mark_pdf_descargado(fal_order_id: str):
    conn = get_conn()
    conn.execute(
        "UPDATE orders SET pdf_descargado=1 WHERE fal_order_id=?",
        (fal_order_id,)
    )
    conn.commit()
    conn.close()
